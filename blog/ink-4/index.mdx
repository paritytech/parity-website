...
Our new format comes with proper documentation and formal specification. You can find the documentation of the new format [here](https://use.ink/4.0.0-alpha.1/metadata). The formal specification is available as a JSON schema [here](https://github.com/paritytech/ink/tree/master/crates/metadata). The formal schema can be used by block explorers to verify contract metadata that is uploaded by users.
...
The general theme is that we made the ink! API more Rust idiomatic, enabling Rust developers a more intuitive usage of ink!. ink! now also supports [a registry for chain extensions](https://github.com/paritytech/chainextension-registry). This enables developers to use extensions of `pallet-contracts`, e.g. to enable smart contracts to access to the `pallet-assets`.
...
We have written a migration guide from ink! 3.x to 4.0. You can find it on our documentation portal [here](https://use.ink/getting-started/migrating-from-ink-3-to-4).
...
Our latest `substrate-contracts-node` (version `v0.23.0`) also supports ink! 4.0.

## What’s next?

The next big item on our roadmap is to integrate support for XCM into ink!. Our current thinking is that we can do this without breaking changes, e.g. with an ink! 4.1 release.
